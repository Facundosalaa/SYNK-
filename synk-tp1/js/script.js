...
function initCatalogoToolbar() {
  const searchInput = $('#buscar-producto');
  const categoriaSelect = $('#filtro-categoria');
  const ordenSelect = $('#orden-precio');
  if (!searchInput && !categoriaSelect && !ordenSelect) return;
  ...
  const apply = () => {
    ...
    const cat  = categoriaSelect?.value || 'todas';
    const ord  = ordenSelect?.value || 'ninguno';

    let filtered = items.filter(el => {
      const nombre = (el.dataset.nombre || el.textContent || '').toLowerCase();
      const categoria = el.dataset.categoria || '';
      const okTerm = !term || nombre.includes(term);
      const okCat  = cat === 'todas' || categoria === cat;
      return okTerm && okCat;
    });

    if (ord !== 'ninguno') {
      filtered.sort((a, b) => {
        const pa = parseInt(a.dataset.precio || '0', 10);
        const pb = parseInt(b.dataset.precio || '0', 10);
    ...
    filtered.forEach(x => { x.style.display = ''; parent && parent.appendChild(x); });
  };

  searchInput && on(searchInput, 'input', debounce(apply, 200));
  categoriaSelect && on(categoriaSelect, 'change', apply);
  ordenSelect && on(ordenSelect, 'change', apply);
}
